          ...
          Box({
            attribute: {
              updateCover: (self) => {
                ...
                const stylePath = '/home/clara/.cache/ags/media/677324f8cb93704510a1d11130f1472eea65009d_colorscheme.css'
                ...
                // Generate colors
                // print(`${App.configDir}/scripts/color_generation/generate_colors_material.py --path '${coverPath}' > ${App.configDir}/scss/_musicmaterial.scss ${lightDark}`)
                // execAsync([
                //   "bash",
                //   "-c",
                //   `${App.configDir}/scripts/color/bin/python ${App.configDir}/scripts/color_generation/generate_colors_material.py -- --path '${coverPath}' > ${App.configDir}/scss/_musicmaterial.scss ${lightDark}`,
                // ])
                //   .then(() => {
                //     exec(
...
export default () =>
  MarginRevealer({
      ...
      setup: (self) =>
        self.hook(Mpris,
          (box) => {
            ...
            Mpris.players.forEach((player, _i) => {
            ...
            if (!foundPlayer) {
              ...
              for (let i = 0; i < children.length; i++) {
                ...
                child.destroy();
                // child = null;
                // Child is const and not assignable
              }
              return;
            }
          },
    ...
    extraSetup: (self) => self.hook(showMusicControls, () => {
      if (showMusicControls.value) self.attribute.show();
      else self.attribute.hide()
    }),
  });
