...
// TODO: sideleft auto closes when lose focus
export const pinned = Variable(false, {})
export const pinButton = Button({
  attribute: {
    toggle: () => {

      const self = pinButton;
      pinned.setValue(!pinned.value);
      self.toggleClassName("sidebar-pin-enabled", pinned.value);
      ...
      sideleftContent.toggleClassName("sidebar-pinned", pinned.value);
...
export const SideLeft = () =>
  Box({
    ...
    children: [
      EventBox({
        // onPrimaryClick: closeSideleft,
        // onSecondaryClick: closeSideleft,
        // onMiddleClick: closeSideleft,
      }),
      Box({
        vertical: true,
        vexpand: true,
        ...
        setup: (self) =>
          self.hook(App, (self, currentName, visible) => {
            if (currentName === "sideleft" && !pinned.value) {
              console.log(`inside toggle visible ${visible} curname ${currentName}`)
              self.toggleClassName(
                "sidebar-pinned",
                pinned.value && visible,
              );
            }
          }),
      }),
