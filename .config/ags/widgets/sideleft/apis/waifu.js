...
async function getImageViewerApp(preferredApp) {
  Utils.execAsync(["bash", "-c", `command -v ${preferredApp}`]).then(
    (output) => {
            ...
            ImageAction({
              ...
              action: () =>
                execAsync([
                  "bash",
                  "-c",
                  `mkdir -p ~/Pictures/homework${thisBlock.attribute.isNsfw ? "/🌶️" : ""} && cp ${thisBlock.attribute.imagePath} ~/Pictures/homework${thisBlock.attribute.isNsfw ? "/🌶️/" : ""}`,
                ]).catch(print),
            }),
            ImageAction({
